    ...
    <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-800 bg-gradient-to-r from-zinc-900/80 via-zinc-900/40 to-zinc-950/90">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={() => {
            if (hasError) {
              alert('Please fix invalid time values in mute segments before rendering.');
              return;
            }
            local.setRenderConfirmOpen(true);
          }}
          disabled={!renderReady || runPipelineSubmitting || hasError}
